import pandas as pd
import json
from api_calls import get_client_data, get_cards_data
...
def get_clients_dataset() -> pd.DataFrame:
    ...
    clients_df["total_debt"] = clients_df["total_debt"].str.replace("$", "")
    clients_df["total_debt"] = pd.to_numeric(clients_df["total_debt"], errors="coerce")
    clients_df["yearly_income"] = clients_df["yearly_income"].str.replace("$", "")
...
def get_cards_dataset() -> pd.DataFrame:
    ...
    cards_df["credit_limit"] = cards_df["credit_limit"].str.replace("$", "")
